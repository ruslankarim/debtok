...
function getData(data) {
    ...
    if (isClientExist(data.email, firestore)) return {message : 'Похоже Вы у нас уже были, напишите нам сюда: debtok@buildocx.ru, для Вас будет создана отдельная форма'}
    ...
    if(dadata.isOrg){
      ...
      firestore.createDocument("clients/" + data.email + '/', toDB)
       firestore.createDocument("clients/" + data.email + '/debtors/' + dadata.inn, dadata)
      MailApp.sendEmail(
        {to: data.email,
